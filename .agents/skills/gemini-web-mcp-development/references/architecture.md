...
`main` contains extensive unreleased changes after the existing `0.2.0` line, including MCP SDK v2 and new result/artifact contracts. The mechanics for single-source versioning and verified release assets exist; the remaining question is product versioning and compatibility communication.
