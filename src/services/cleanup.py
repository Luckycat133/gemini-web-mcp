...
async def cleanup_test_artifacts_payload(
    ...
    if include_chats:
        if not hasattr(client, "list_chats"):
            ...
            chats = (client.list_chats() or [])[:safe_chat_limit]  # type: ignore[attr-defined]
            for chat in chats:
                item = chat_to_dict(chat)
                matched_fields: list[str] = []
                matched_markers = marker_hits(item.get("id"), marker_list)
                if matched_markers:
                    matched_fields.append("id")
                title_hits = marker_hits(item.get("title"), marker_list)
                if title_hits:
                ...
                if scan_turns and item.get("id") and hasattr(client, "read_chat"):
                    try:
                        ...
                        for turn in turns:
                            hits = marker_hits(turn.get("text"), marker_list)
                            if hits:
        ...
        if not hasattr(client, "_batch_execute"):
            ...
            try:
                ...
                for item in entries:
                    ...
                    matched_markers = marker_hits(search_text, marker_list)
